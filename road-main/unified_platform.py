...
.wx-banner{
  display:flex;align-items:center;gap:12px;
  background:linear-gradient(90deg,#0a1a2e,#0d2040);
  border:1px solid #1e3a5f;border-radius:10px;
  padding:10px 16px;margin-bottom:12px;
}
.wx-icon{font-size:1.6rem;}
.wx-temp{font-size:1.4rem;font-weight:800;color:#60a5fa;}
.wx-desc{font-size:.78rem;color:#94a3b8;text-transform:capitalize;}
.wx-detail{font-size:.72rem;color:#64748b;margin-top:2px;}
.wx-alert{
...
.wx-alert.warn{
  background:rgba(251,191,36,.08);border-color:#92400e;color:#fbbf24;
}
.wx-alert.danger{
  background:rgba(239,68,68,.1);border-color:#991b1b;color:#f87171;
}
.wx-penalty{
  font-size:.72rem;color:#f97316;font-weight:700;
  background:rgba(249,115,22,.1);border:1px solid #7c2d12;
  border-radius:4px;padding:2px 7px;display:inline-block;margin-top:4px;
}
</style>
...
# ─── Weather Integration ───────────────────────────────────────────────────────
# Open-Meteo free API — no key required, covers Bangalore.
# Latitude/Longitude: Bangalore city centre.
BANGALORE_LAT  = 12.9716
BANGALORE_LON  = 77.5946

# WMO weather code → (description, icon, severity)
# severity: 'clear'|'cloudy'|'drizzle'|'rain'|'heavy_rain'|'storm'|'fog'
WMO_CODES = {
    0:  ("Clear Sky",           "☀️",  "clear"),
    ...
    65: ("Heavy Rain",          "🌧️",  "heavy_rain"),
    71: ("Slight Snow",         "❄️",  "rain"),
    73: ("Moderate Snow",       "❄️",  "heavy_rain"),
    75: ("Heavy Snow",          "❄️",  "heavy_rain"),
    77: ("Snow Grains",         "🌨️",  "rain"),
    80: ("Slight Showers",      "🌦️",  "drizzle"),
    81: ("Moderate Showers",    "🌧️",  "rain"),
    82: ("Violent Showers",     "⛈️",  "heavy_rain"),
    95: ("Thunderstorm",        "⛈️",  "storm"),
    96: ("Thunderstorm + Hail", "⛈️",  "storm"),
    99: ("Severe Thunderstorm", "🌩️",  "storm"),
}

# Speed penalty (km/h to deduct) per severity × road type
# Vision Zero principle: wet/low-vis roads need conservative adjustments
WEATHER_PENALTY = {
    #               Highway  Arterial  Collector  Local
    "clear":     {  "Highway": 0,  "Arterial": 0,  "Collector": 0,  "Local": 0  },
    "cloudy":    {  "Highway": 0,  "Arterial": 0,  "Collector": 0,  "Local": 0  },
    "drizzle":   {  "Highway": 5,  "Arterial": 5,  "Collector": 5,  "Local": 5  },
    "rain":      {  "Highway":10,  "Arterial":10,  "Collector": 8,  "Local": 5  },
    "heavy_rain":{  "Highway":15,  "Arterial":15,  "Collector":12,  "Local":10  },
    "storm":     {  "Highway":20,  "Arterial":20,  "Collector":15,  "Local":10  },
    "fog":       {  "Highway":15,  "Arterial":10,  "Collector": 8,  "Local": 5  },
}

# Exposure multiplier for temporal_exposure() under wet/low-vis conditions
WEATHER_EXPOSURE_MULT = {
    "clear":      1.0,
    "cloudy":     1.0,
    "drizzle":    1.15,
    "rain":       1.25,
...
def fetch_bangalore_weather():
    """
    Fetch current weather from Open-Meteo for Bangalore.
    Returns a dict with keys: temp_c, feels_like_c, humidity, wind_kph,
    precip_mm, wmo_code, description, icon, severity, vis_km, uv_index.
    Falls back to a safe 'clear' default if the API is unreachable.
    """
    default = {
        "temp_c": None, "feels_like_c": None, "humidity": None,
        "wind_kph": None, "precip_mm": 0.0,
        "wmo_code": 0, "description": "Unknown (offline)", "icon": "❓",
        "severity": "clear", "vis_km": None, "uv_index": None,
        "fetched_ok": False,
    }
    if not REQUESTS_OK:
        return default
    try:
        url = (
            ...
            "precipitation,weather_code,wind_speed_10m,visibility,uv_index"
        ...
        desc, icon, sev = WMO_CODES.get(code, ("Unknown", "❓", "clear"))
        # Open-Meteo returns visibility in metres
        vis_raw = c.get("visibility")
        vis_km  = round(vis_raw / 1000, 1) if vis_raw is not None else None
        # Upgrade severity to fog if visibility < 1 km regardless of WMO code
        if vis_km is not None and vis_km < 1.0 and sev == "clear":
            sev, desc, icon = "fog", "Dense Fog", "🌫️"
        return {
            "temp_c":       round(c.get("temperature_2m", 28), 1),
            "feels_like_c": round(c.get("apparent_temperature", 28), 1),
            "humidity":     int(c.get("relative_humidity_2m", 60)),
            "wind_kph":     round(c.get("wind_speed_10m", 0), 1),
            "precip_mm":    round(c.get("precipitation", 0.0), 1),
            "wmo_code":     code,
            "description":  desc,
            "icon":         icon,
            "severity":     sev,
...
def weather_speed_penalty(road_type: str, severity: str) -> int:
    """Return km/h to subtract from recommended speed for this weather + road combo."""
    rt = road_type if road_type in ("Highway","Arterial","Collector","Local") else "Local"
    return WEATHER_PENALTY.get(severity, WEATHER_PENALTY["clear"]).get(rt, 0)

def weather_exposure_mult(severity: str) -> float:
    return WEATHER_EXPOSURE_MULT.get(severity, 1.0)

def weather_alert_level(severity: str) -> str:
    """Return 'none'|'info'|'warn'|'danger' for banner styling."""
    return {"clear":"none","cloudy":"none","drizzle":"info",
            "rain":"warn","heavy_rain":"danger","storm":"danger","fog":"warn"}.get(severity,"none")

# ── Road-condition severity ordering ──────────────────────────────────────────
SEV_RANK = {"clear":0,"cloudy":1,"drizzle":2,"rain":3,"heavy_rain":4,"storm":5,"fog":3}
SEV_NAMES = {0:"clear",1:"cloudy",2:"drizzle",3:"rain",4:"heavy_rain",5:"storm"}

def _sev_rank(s): return SEV_RANK.get(s, 0)

# Per-road-type historical baseline means (computed at startup from the dataset;
# used as the denominator for speed-depression detection).
# These are populated in compute_speed_baselines() below.
SPEED_BASELINES: dict = {}   # {road_type: {"mean": float, "p85": float, "var": float}}

def compute_speed_baselines(df_full):
    """
    Compute per-road-type baseline operating speeds from the loaded dataset.
    Called once at startup. Stores results into SPEED_BASELINES.
    """
    global SPEED_BASELINES
    for rt, grp in df_full.groupby("road_type"):
        SPEED_BASELINES[rt] = {
            "mean": float(grp["operating_speed_mean"].median()),
            "p85":  float(grp["speed_p85"].median()),
            "var":  float((grp["speed_p85"] - grp["operating_speed_mean"]).median()),
        }

def speed_depression_index(row) -> float:
    """
    How much slower is this segment running compared to its road-type baseline?
    Returns 0.0–1.0, where 1.0 = completely stopped, 0.0 = at or above baseline.
    Positive values indicate real-world slowdown regardless of cause.
    """
    rt   = str(row.get("road_type", "Local"))
    base = SPEED_BASELINES.get(rt, {}).get("mean", 30.0)
    curr = float(row.get("operating_speed_mean", base))
    return max(0.0, (base - curr) / base)

def speed_variance_signal(row) -> float:
    """
    Normalised speed variance: (p85 - mean) / baseline_variance.
    High values → heterogeneous speeds → conditions affecting some drivers more
    than others (wet lanes, visibility patches, etc.).
    """
    rt       = str(row.get("road_type", "Local"))
    base_var = SPEED_BASELINES.get(rt, {}).get("var", 9.7)  # network median fallback
    curr_var = float(row.get("speed_p85", 40)) - float(row.get("operating_speed_mean", 30))
    return min(2.0, curr_var / max(base_var, 1.0))

def fused_condition_severity(row, api_severity: str) -> tuple[str, float, str]:
    """
    Fuse the API weather forecast with real-world probe speed signals to produce
    a ground-truth-anchored road condition severity estimate.

    Returns:
        (fused_severity, confidence, explanation)

    Logic:
    ──────
    • API signal        → base severity from WMO code (low trust alone)
    • Speed depression  → how much slower traffic is vs historical baseline
                          (strong signal; drivers respond to actual conditions)
    • Speed variance    → heterogeneous speeds often precede or accompany rain/fog
                          (moderate signal; also correlates with incidents)

    Confidence is HIGH when API and probe signals agree, LOW when they diverge.
    When they diverge, the probe signal is trusted MORE because it reflects what
    is actually happening on the road surface right now — not a forecast.

    Speed-depression thresholds (calibrated to Bangalore data distribution):
        ≥ 0.30  → heavy rain / severe degradation
        ≥ 0.20  → rain
        ≥ 0.12  → drizzle / light degradation
        < 0.12  → normal / clear (baseline noise)
    """
    dep = speed_depression_index(row)
    var = speed_variance_signal(row)

    # Infer probe-implied severity from speed depression
    if dep >= 0.30:
        probe_sev = "heavy_rain"
    elif dep >= 0.20:
        probe_sev = "rain"
    elif dep >= 0.12:
        probe_sev = "drizzle"
    ...
    # Variance bonus: high variance can push severity up one notch
    if var >= 1.4 and _sev_rank(probe_sev) < _sev_rank("rain"):
        probe_sev = SEV_NAMES.get(min(5, _sev_rank(probe_sev) + 1), probe_sev)

    api_rank   = _sev_rank(api_severity)
    probe_rank = _sev_rank(probe_sev)
    diff       = abs(api_rank - probe_rank)

    # Agreement → high confidence; use the higher of the two (conservative)
    if diff == 0:
        fused = api_severity
        confidence = 0.90
        explanation = f"API ({api_severity}) and probe data agree → high confidence"
    elif diff == 1:
        # Small disagreement — lean toward probe but blend conservatively
        fused = SEV_NAMES.get(max(api_rank, probe_rank), api_severity)
        confidence = 0.70
        explanation = (f"Minor divergence: API={api_severity}, probe={probe_sev} "
                       f"(dep={dep:.0%}) → using worse of the two")
    else:
        # Significant disagreement — probe wins; API may be wrong or stale
        fused = probe_sev
        confidence = 0.50
        explanation = (f"API={api_severity} but probe data shows dep={dep:.0%} "
                       f"→ real-world signal overrides forecast (low confidence in API)")

    return fused, confidence, explanation

def fused_speed_penalty(row, api_severity: str) -> tuple[int, float, str]:
    """
    Return (penalty_kmh, confidence, explanation) for a segment, using
    the fused condition severity rather than the raw API severity alone.
    """
    fused_sev, conf, expl = fused_condition_severity(row, api_severity)
    rt = str(row.get("road_type", "Local"))
    penalty = weather_speed_penalty(rt, fused_sev)
    return penalty, conf, f"[{fused_sev}@{conf:.0%}] {expl}"
...
compute_speed_baselines(df)   # populate SPEED_BASELINES for fusion model
...
# ── Fetch live weather ────────────────────────────────────────────────────────
wx = fetch_bangalore_weather()
WX_SEV = wx["severity"]   # global shorthand used in speed/exposure calcs
...
def apply_dynamic_hazard_speeds():
    ...
    # ── Fused weather speed penalty (per segment, API + probe data) ───────────
    # Instead of applying a flat penalty from the API forecast, we compute a
    # per-segment fused condition severity that cross-checks the API against
    # real-world GPS probe speed depression on that segment.
    # This means a segment running at normal speeds despite rain forecast gets
    # a lower penalty, while a segment with anomalous speed depression gets a
    # higher penalty even if the API says clear.
    def _row_fused_penalty(row):
        pen, conf, _ = fused_speed_penalty(row, WX_SEV)
        return pen
    def _row_fused_conf(row):
        _, conf, _ = fused_speed_penalty(row, WX_SEV)
        return conf
    def _row_fused_sev(row):
        sev, _, _ = fused_condition_severity(row, WX_SEV)
        return sev

    df['weather_penalty']   = df.apply(_row_fused_penalty, axis=1)
    df['wx_confidence']     = df.apply(_row_fused_conf,    axis=1)
    df['fused_wx_severity'] = df.apply(_row_fused_sev,     axis=1)
    df['speed_depression']  = df.apply(speed_depression_index, axis=1).round(3)
    df['weather_adjusted_speed'] = (
        df['recommended_safe_speed'] - df['weather_penalty']
    ).clip(lower=15).round().astype(int)
    df['recommended_safe_speed'] = df['weather_adjusted_speed']
...
def temporal_exposure(row, hour):
    ...
    # Use the per-segment fused severity if already computed, else fall back to API
    row_sev  = str(row.get('fused_wx_severity', WX_SEV))
    wx_m     = weather_exposure_mult(row_sev)
    return min(100, base * school_m * peak_m * night_m * market_m * wx_m)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### 🌦️ Live Weather — Bangalore")
    _penalty_eg = weather_speed_penalty("Highway", WX_SEV)
    _alert      = weather_alert_level(WX_SEV)
    _alert_cls  = {"none":"","info":"","warn":" warn","danger":" danger"}.get(_alert,"")
    _wx_rows = ""
    if wx["temp_c"] is not None:
        _wx_rows += f'<div class="irow"><span class="irow-l">Temperature</span><span class="irow-v">{wx["temp_c"]}°C (feels {wx["feels_like_c"]}°C)</span></div>'
    if wx["humidity"] is not None:
        _wx_rows += f'<div class="irow"><span class="irow-l">Humidity</span><span class="irow-v">{wx["humidity"]}%</span></div>'
    if wx["wind_kph"] is not None:
        _wx_rows += f'<div class="irow"><span class="irow-l">Wind</span><span class="irow-v">{wx["wind_kph"]} km/h</span></div>'
    if wx["precip_mm"] is not None:
        _wx_rows += f'<div class="irow"><span class="irow-l">Precipitation</span><span class="irow-v">{wx["precip_mm"]} mm</span></div>'
    if wx["vis_km"] is not None:
        _wx_rows += f'<div class="irow"><span class="irow-l">Visibility</span><span class="irow-v">{wx["vis_km"]} km</span></div>'
    _offline_note = "" if wx["fetched_ok"] else '<div style="color:#64748b;font-size:.68rem;margin-top:4px">⚠️ Weather data unavailable — no speed adjustments applied.</div>'
    _penalty_note = (f'<div class="wx-penalty">🚦 Fused penalty: −{_penalty_eg} km/h on Highways (API + probe)</div>'
                     if _penalty_eg > 0 else
                     '<div style="color:#22c55e;font-size:.72rem;margin-top:4px">✓ No speed penalty (fused API + probe)</div>')
    st.markdown(f"""
    <div style="background:#0d1b2e;border-radius:10px;padding:12px;border:1px solid #1a2d45">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
        <span style="font-size:1.8rem">{wx["icon"]}</span>
        <div>
          <div style="color:#e2e8f0;font-weight:700;font-size:.92rem">{wx["description"]}</div>
          <div style="color:#64748b;font-size:.7rem;text-transform:uppercase;letter-spacing:.06em">{WX_SEV}</div>
        </div>
      </div>
      {_wx_rows}
      {_penalty_note}
...
# ─── Weather Alert Banner (shown when conditions affect road safety) ──────────
_wx_alert = weather_alert_level(WX_SEV)
if _wx_alert != "none":
    _banner_cls = "warn" if _wx_alert in ("warn","info") else "danger"
    _penalty_hw = weather_speed_penalty("Highway", WX_SEV)
    _penalty_ar = weather_speed_penalty("Arterial", WX_SEV)
    _banner_msg = {
        "info":   f"🌦️ <b>Light rain / drizzle</b> detected — speeds reduced by {_penalty_ar}–{_penalty_hw} km/h on affected road types.",
        "warn":   f"🌧️ <b>{wx['description']}</b> — wet road conditions active. Speed penalties: Highway −{_penalty_hw} km/h · Arterial −{_penalty_ar} km/h.",
        "danger": f"⛈️ <b>Severe weather: {wx['description']}</b> — significant speed reductions applied network-wide. Highway −{_penalty_hw} km/h · Arterial −{_penalty_ar} km/h. Exercise extreme caution.",
    }.get(_banner_cls, "")
    st.markdown(
        f'<div class="wx-alert {_banner_cls}">{_banner_msg} &nbsp;|&nbsp; '
        f'Visibility: {wx["vis_km"]} km &nbsp;|&nbsp; Precip: {wx["precip_mm"]} mm</div>',
        unsafe_allow_html=True)
...
tab_map, tab_hot, tab_model, tab_xai, tab_crash, tab_hazard, tab_wx, tab_analytics, tab_data = tabs
...
with tab_map:
    ...
    with map_col:
        ...
        for feat in geojson["features"]:
            ...
            if has_hazard and hz_temp_spd_map:
                ...
                hazard_banner = ''
                _map_wx_pen = int(p.get('weather_penalty', 0))
                _map_fused_sev = str(p.get('fused_wx_severity', WX_SEV))
                _map_conf = float(p.get('wx_confidence', 0.5))
                _wx_suffix  = (f" | {wx['icon']} {_map_fused_sev} −{_map_wx_pen} km/h "
                               f"[{_map_conf:.0%} conf]") if _map_wx_pen > 0 else ""
                speed_calc_note = f"Vision Zero: min(AI={p.get('ai_recommended_speed',p.get('final_safe_speed', p['recommended_safe_speed']))} km/h, Tolerance={p.get('human_tolerance_limit',70):.0f} km/h) → {p.get('final_safe_speed', p['recommended_safe_speed'])} km/h{_wx_suffix}"
    ...
    with detail_col:
        ...
        if not seg_opts:
            ...
            wx_pen      = int(r.get('weather_penalty', 0))
            wx_conf     = float(r.get('wx_confidence', 1.0))
            wx_fused    = str(r.get('fused_wx_severity', WX_SEV))
            wx_dep      = float(r.get('speed_depression', 0.0))
            ...
            _conf_color = "#22c55e" if wx_conf >= 0.85 else "#fbbf24" if wx_conf >= 0.65 else "#f87171"
            _dep_pct = f"{wx_dep:.0%}"
            wx_line = (
                f'<div class="wx-penalty">'
                f'{wx["icon"]} Condition: <b>{wx_fused.replace("_"," ")}</b> '
                f'(API: {wx["description"]}) · Penalty: −{wx_pen} km/h'
                f'</div>'
                f'<div style="font-size:.68rem;color:{_conf_color};margin-top:3px">'
                f'Fusion confidence: {wx_conf:.0%} &nbsp;|&nbsp; '
                f'Speed depression vs baseline: {_dep_pct}'
                f'</div>'
                if wx_pen > 0 and not hz_temp_spd else
                (f'<div style="color:#22c55e;font-size:.7rem;margin-top:4px">'
                 f'✓ No weather penalty · Speed depression: {_dep_pct} · Conf: {wx_conf:.0%}</div>'
                 if not hz_temp_spd else "")
            )
            ...
            _wx_factor_tag = (
                f'<div class="factor-pill f-high">'
                f'{wx["icon"]} Road: {wx_fused.replace("_"," ")} [{wx_conf:.0%} conf] — −{wx_pen} km/h</div>'
                if wx_pen > 0 else "")
            _wx_vz_note = (f'<br><b>Weather Adjustment:</b> {wx["description"]} → −{wx_pen} km/h on {r.get("road_type","")}'
                           if wx_pen > 0 else "")
            st.markdown(f"""
            <div class="xai-box">
...
# ══════════════════════════════════════════════════════════════════════════════
with tab_wx:
    st.markdown("## 🌦️ Weather Impact on Road Safety")

    # ── Current conditions card ───────────────────────────────────────────────
    wc1, wc2, wc3, wc4, wc5 = st.columns(5)
    wx_kpis = [
        (f"{wx['temp_c']}°C",     "Temperature",    "blue"),
        (f"{wx['humidity']}%",    "Humidity",        "sky"),
        (f"{wx['wind_kph']} km/h","Wind Speed",      "teal"),
        (f"{wx['precip_mm']} mm", "Precipitation",   "purple"),
        (f"{wx['vis_km']} km" if wx['vis_km'] is not None else "—", "Visibility", "orange"),
    ]
    for col_w, (val, lbl, cls) in zip([wc1,wc2,wc3,wc4,wc5], wx_kpis):
        col_w.markdown(
            f'<div class="kpi"><div class="kpi-v {cls}">{val}</div>'
            f'<div class="kpi-l">{lbl}</div></div>',
            unsafe_allow_html=True)
    ...
    _alert_lv = weather_alert_level(WX_SEV)
    _sev_color = {"clear":"#22c55e","cloudy":"#94a3b8","drizzle":"#60a5fa",
                  "rain":"#fbbf24","heavy_rain":"#f97316","storm":"#ef4444","fog":"#a78bfa"}.get(WX_SEV,"#94a3b8")
    st.markdown(f"""
    <div style="background:#0d1b2e;border:1px solid #1a2d45;border-radius:12px;padding:16px;margin-bottom:16px;display:flex;align-items:center;gap:20px">
      <div style="font-size:3rem">{wx['icon']}</div>
      <div>
        <div style="font-size:1.3rem;font-weight:800;color:{_sev_color}">{wx['description']}</div>
        <div style="color:#64748b;font-size:.8rem;margin-top:4px">Severity class: <b style="color:{_sev_color}">{WX_SEV.replace('_',' ').upper()}</b></div>
        <div style="color:#94a3b8;font-size:.78rem;margin-top:4px">
          Weather data refreshes every 15 minutes · Source: Open-Meteo (Bangalore 12.97°N 77.59°E)
          {'&nbsp;·&nbsp;<span style="color:#22c55e">✓ Live</span>' if wx['fetched_ok'] else '&nbsp;·&nbsp;<span style="color:#ef4444">Offline — using defaults</span>'}
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)

    # ── Speed penalty table by road type ─────────────────────────────────────
    st.markdown("### 🚦 Speed Adjustments by Road Type")
    st.caption("Penalties applied to the AI-recommended speed in real time based on current conditions.")

    road_types_wx = ["Highway", "Arterial", "Collector", "Local"]
    severities_wx = ["clear", "drizzle", "rain", "heavy_rain", "storm", "fog"]
    sev_labels    = ["Clear ☀️", "Drizzle 🌦️", "Rain 🌧️", "Heavy Rain 🌧️", "Storm ⛈️", "Fog 🌫️"]

    penalty_rows = []
    for rt in road_types_wx:
        row_d = {"Road Type": rt}
        for sev, lbl in zip(severities_wx, sev_labels):
            p = weather_speed_penalty(rt, sev)
            row_d[lbl] = f"−{p} km/h" if p > 0 else "None"
        penalty_rows.append(row_d)
    penalty_df = pd.DataFrame(penalty_rows).set_index("Road Type")

    # Highlight current severity column
    current_lbl = sev_labels[severities_wx.index(WX_SEV)] if WX_SEV in severities_wx else None

    def _style_wx(val):
        if val == "None":
            return "color:#22c55e"
        p = int(val.replace("−","").replace(" km/h",""))
        if p >= 15: return "color:#ef4444;font-weight:700"
        if p >= 10: return "color:#f97316;font-weight:700"
        if p >= 5:  return "color:#fbbf24"
        return "color:#94a3b8"

    st.dataframe(penalty_df.style.applymap(_style_wx), use_container_width=True)

    # ── Network impact summary ────────────────────────────────────────────────
    st.markdown("### 📊 Network-Wide Fusion Model Results")

    # Add KPIs for the fusion model state
    if 'wx_confidence' in df.columns and 'fused_wx_severity' in df.columns:
        _n_agree      = int((df['fused_wx_severity'] == WX_SEV).sum())
        _n_api_higher = int((df.apply(lambda r: _sev_rank(str(r.get('fused_wx_severity',WX_SEV))) < _sev_rank(WX_SEV), axis=1)).sum())
        _n_probe_higher = int((df.apply(lambda r: _sev_rank(str(r.get('fused_wx_severity',WX_SEV))) > _sev_rank(WX_SEV), axis=1)).sum())
        _avg_conf     = df['wx_confidence'].mean()
        _avg_dep      = df['speed_depression'].mean() if 'speed_depression' in df.columns else 0
        fkpi1, fkpi2, fkpi3, fkpi4, fkpi5 = st.columns(5)
        for col_w, (val, lbl, cls) in zip(
            [fkpi1,fkpi2,fkpi3,fkpi4,fkpi5],
            [
                (f"{_n_agree:,}",        "Segments: API=Probe",    "green"),
                (f"{_n_api_higher:,}",   "Probe > API (API overestimates)", "orange"),
                (f"{_n_probe_higher:,}", "Probe < API (Worse on ground)",   "red"),
                (f"{_avg_conf:.0%}",     "Avg Fusion Confidence",  "blue"),
                (f"{_avg_dep:.1%}",      "Avg Speed Depression",   "purple"),
            ]
        ):
            col_w.markdown(
                f'<div class="kpi"><div class="kpi-v {cls}">{val}</div>'
    ...
    wx2a, wx2b = st.columns(2)

    with wx2a:
        # Fused severity distribution vs API severity
        if 'fused_wx_severity' in df.columns:
            sev_counts = df['fused_wx_severity'].value_counts().reset_index()
            sev_counts.columns = ['Severity','Segments']
            sev_color_map = {
                "clear":"#22c55e","cloudy":"#94a3b8","drizzle":"#60a5fa",
                "rain":"#fbbf24","heavy_rain":"#f97316","storm":"#ef4444","fog":"#a78bfa"
            }
            fig_sev = px.bar(
                sev_counts.sort_values('Segments', ascending=True),
                x='Segments', y='Severity', orientation='h',
                color='Severity', color_discrete_map=sev_color_map,
                title=f"Fused Road Condition (API said: {WX_SEV})",
                template="plotly_dark")
            fig_sev.update_layout(height=300, paper_bgcolor=BG, plot_bgcolor=BG,
                                   margin=dict(t=40,b=10,l=0,r=10), showlegend=False,
                                   xaxis=dict(color='#64748b'), yaxis=dict(color='#94a3b8'))
            st.plotly_chart(fig_sev, use_container_width=True)

    with wx2b:
        # Before vs after speed comparison using fused penalties
        if 'ai_recommended_speed' in df.columns and 'weather_penalty' in df.columns:
            before_avg = df.groupby('road_type')['ai_recommended_speed'].mean().round(1)
            after_avg  = (df['ai_recommended_speed'] - df['weather_penalty']).clip(lower=15).groupby(df['road_type']).mean().round(1)
            comp_df = pd.DataFrame({
                'Before (AI baseline)': before_avg,
                'After (Fused weather)': after_avg,
            }).reset_index()
            fig_comp = go.Figure()
            fig_comp.add_bar(x=comp_df['road_type'], y=comp_df['Before (AI baseline)'],
                              name='AI Baseline', marker_color='#60a5fa')
            fig_comp.add_bar(x=comp_df['road_type'], y=comp_df['After (Fused weather)'],
                              name='After Fusion', marker_color='#f97316')
            fig_comp.update_layout(
                barmode='group', title='Avg Safe Speed: Before vs After Fused Adjustment',
                height=300, paper_bgcolor=BG, plot_bgcolor=BG,
                margin=dict(t=40,b=10,l=0,r=0), template='plotly_dark',
                legend=dict(font=dict(size=9), bgcolor='rgba(0,0,0,0)'),
                xaxis=dict(color='#64748b'), yaxis=dict(color='#64748b',title='km/h'))
            st.plotly_chart(fig_comp, use_container_width=True)

    # ── Confidence scatter: depression vs penalty, coloured by fusion agreement ──
    st.markdown("### 🔬 Fusion Diagnostics — Speed Depression vs Penalty")
    st.caption("Each point is a road segment. X axis = how much slower traffic is running vs historical baseline. "
               "Y axis = speed penalty applied. Colour = fusion confidence. "
               "Segments in the top-right are being caught by real-world probe data, "
               "not just the API forecast.")
    if all(c in df.columns for c in ['speed_depression','weather_penalty','wx_confidence','fused_wx_severity']):
        fig_diag = px.scatter(
            df,
            x='speed_depression', y='weather_penalty',
            color='wx_confidence',
            color_continuous_scale='RdYlGn',
            size='road_risk_score',
            hover_name='road_name',
            hover_data=['road_type','fused_wx_severity','operating_speed_mean','speed_p85'],
            template='plotly_dark',
            labels={
                'speed_depression': 'Speed Depression (vs baseline)',
                'weather_penalty':  'Applied Penalty (km/h)',
                'wx_confidence':    'Fusion Confidence',
            }
        )
        # Mark API-only threshold line
        fig_diag.add_vline(x=0.12, line_dash="dash", line_color="#fbbf24",
                            annotation_text="Drizzle threshold", annotation_font_size=9,
                            annotation_font_color="#fbbf24")
        fig_diag.add_vline(x=0.20, line_dash="dash", line_color="#f97316",
                            annotation_text="Rain threshold", annotation_font_size=9,
                            annotation_font_color="#f97316")
        fig_diag.add_vline(x=0.30, line_dash="dash", line_color="#ef4444",
                            annotation_text="Heavy rain threshold", annotation_font_size=9,
                            annotation_font_color="#ef4444")
        fig_diag.update_layout(
            height=380, paper_bgcolor=BG, plot_bgcolor=BG,
            margin=dict(t=20,b=40,l=10,r=10),
            xaxis=dict(color='#64748b', tickformat='.0%'),
            yaxis=dict(color='#64748b'),
            coloraxis_colorbar=dict(
                title="Confidence", tickformat='.0%',
                tickfont=dict(color='#94a3b8',size=8)))
        st.plotly_chart(fig_diag, use_container_width=True)

    # ── High-risk-in-weather segments table ───────────────────────────────────
    st.markdown("### ⚠️ Segments Most Affected by Current Conditions")
    st.caption("Ranked by risk score. Shows both the API forecast and the fused condition actually used for speed adjustment.")
    if 'weather_penalty' in df.columns:
        wx_risk = df.copy()
        wx_risk['wx_adjusted_speed'] = (
            wx_risk['ai_recommended_speed'] - wx_risk['weather_penalty']
        ).clip(lower=15).astype(int)
        wx_top = wx_risk.nlargest(25, 'road_risk_score')[
            ['human_segment_id','road_name','road_type',
             'ai_recommended_speed','weather_penalty','wx_adjusted_speed',
             'fused_wx_severity','wx_confidence','speed_depression',
             'road_risk_score','hotspot_category']
        ].rename(columns={
            'ai_recommended_speed': 'Base Speed',
            'weather_penalty':      'Penalty',
            'wx_adjusted_speed':    'Final Speed',
            'fused_wx_severity':    'Fused Condition',
            'wx_confidence':        'Confidence',
            'speed_depression':     'Speed Depression',
            'road_risk_score':      'Risk Score',
        })
        wx_top['Confidence'] = wx_top['Confidence'].map(lambda x: f"{x:.0%}")
        wx_top['Speed Depression'] = wx_top['Speed Depression'].map(lambda x: f"{x:.1%}")
        st.dataframe(wx_top, use_container_width=True, height=360)
        if int((df['weather_penalty'] > 0).sum()) == 0:
            st.success("✅ No speed penalties active — all fused conditions are 'clear'.")

    # ── Vision Zero + Fusion rationale ────────────────────────────────────────
    _api_note = f"API forecast says <b>{WX_SEV}</b>."
    st.markdown(f"""
    <div class="vz-box" style="margin-top:8px">
      <b>Fusion Model Rationale</b><br>
      {_api_note} Weather forecasts are probabilistic and often wrong at the segment level.
      This platform cross-checks the API against <b>actual GPS probe speeds</b> on each segment:
      if drivers are measurably slower than the historical baseline for that road type,
      the probe signal takes precedence — because it reflects what is happening on the road
      surface <em>right now</em>, not what was predicted hours ago.<br><br>
      <b>Speed depression ≥ 30%</b> → heavy rain conditions applied regardless of API.<br>
      <b>Speed depression 20–30%</b> → rain conditions.<br>
      <b>Speed depression 12–20%</b> → drizzle / light degradation.<br>
      <b>Confidence is HIGH</b> when API and probe agree; <b>LOW</b> when they significantly diverge
      (the probe wins in that case, but the low confidence flag signals that ground-truth validation is needed).
    </div>
    """, unsafe_allow_html=True)
